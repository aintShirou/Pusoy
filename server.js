...
function createRoom(id, mode='dos'){ return {id, mode, players:[], started:false, hands:{}, turn:0, current:null, passes:0, lastPlayer:null, winner:null, round:1, firstPlay:true, botTimer:null, finished:[], gameOver:false}; }
function publicRoom(room){ return {id:room.id, mode:room.mode, started:room.started, players:room.players.map(p=>({id:p.id,name:p.name,cardCount:room.hands[p.id]?.length||0,ready:!!p.ready,isBot:!!p.isBot,eliminated:!!p.eliminated})), turn:room.players[room.turn]?.id||null, current:room.current, winner:room.winner, round:room.round, finished:room.finished, gameOver:room.gameOver}; }
...
function nextActiveIndex(room, idx){
  if(!room.players.length) return 0;
  for(let step=1;step<=room.players.length;step++){ const i=(idx+step)%room.players.length; if(!room.players[i].eliminated) return i; }
  return idx;
}
...
function botTurn(roomId){
  const room=rooms.get(roomId); if(!room || !room.started || room.gameOver) return;
  const bot=room.players[room.turn]; if(!bot?.isBot || bot.eliminated) return;
  ...
  if(choice){
    ...
    if(room.hands[bot.id].length===0){
      finishPlayer(room, bot.id);
      if(room.gameOver){ broadcastState(room); return; }
      room.current=null; room.passes=0; room.lastPlayer=null;
      room.turn=nextActiveIndex(room, room.turn);
    } else {
      room.turn=nextActiveIndex(room,room.turn);
    }
  } else if(room.current){
    room.passes++; room.turn=nextActiveIndex(room,room.turn);
    if(room.passes>=activeCount(room)-1){
      const li=room.players.findIndex(x=>x.id===room.lastPlayer && !x.eliminated);
      room.turn=li<0?nextActiveIndex(room,room.turn):li; room.current=null; room.passes=0;
    }
  } else {
...
function startDos(room){
  ...
  room.started=true; room.current=null; room.passes=0; room.lastPlayer=null; room.winner=null; room.round=1; room.firstPlay=true; room.finished=[]; room.gameOver=false;
...
function finishPlayer(room, playerId){
  ...
  p.eliminated=true;
  room.finished.push(playerId);
  room.winner=playerId;
  if(activeCount(room)<=1){
    const last=room.players.find(x=>!x.eliminated);
    if(last){ room.finished.push(last.id); }
    room.gameOver=true;
    room.started=false;
    room.current=null;
    room.passes=0;
...
io.on('connection', socket=>{
  ...
  socket.on('game:playAgain',()=>{
    ...
    if(room.players[0]?.id!==socket.id) return socket.emit('error:msg','Only the host can start the next game.');
    if(room.players.length<2) return socket.emit('error:msg','Need at least 2 players.');
    if(room.mode!=='dos') return socket.emit('error:msg','This build currently ships the real-time Pusoy Dos table.');
    resetForPlayAgain(room);
    broadcastState(room);
    scheduleBot(room);
  });
  socket.on('game:play', ids=>{
    const room=rooms.get(socket.data.room); if(!room||!room.started||room.gameOver) return;
  ...
  socket.on('game:pass',()=>{
    ...
    room.passes++; room.turn=nextActiveIndex(room,room.turn);
    if(room.passes>=activeCount(room)-1){ const li=room.players.findIndex(x=>x.id===room.lastPlayer && !x.eliminated); room.turn=li<0?nextActiveIndex(room,room.turn):li; room.current=null; room.passes=0; }
    broadcastState(room); scheduleBot(room);
  });
  socket.on('disconnect',()=>leaveRoom(socket));
});
function join(socket,room,name){ name=String(name||'Player').trim().slice(0,18)||'Player'; const p={id:socket.id,name,ready:true,isBot:false,eliminated:false}; room.players.push(p); socket.join(room.id); socket.data.room=room.id; socket.emit('joined',{roomId:room.id}); broadcastState(room); }
