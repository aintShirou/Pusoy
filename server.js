...
function finishBetting(room){
  room.betting=false;room.betCurrent=0;room.betContrib={};
...
function startBetting(room){
  room.betting=true;room.betCurrent=room.ante;room.betContrib={};
...
function handleBet(room,p,action){
  ...
  if(action==='fold'){p.folded=true;room.betTurn=nextBetIndex(room,room.betTurn)}
  else if(action==='call'){const add=Math.max(0,room.betCurrent-cur);if(add>p.chips)return{ok:false,msg:'Not enough chips to call.'};p.chips-=add;room.pot+=add;room.betContrib[p.id]=cur+add;room.betTurn=nextBetIndex(room,room.betTurn)}
  else if(action==='raise'){if(room.betRaised)return{ok:false,msg:'Only one raise is allowed in this betting round.'};const target=room.betCurrent+room.betSize;const add=target-cur;if(add>p.chips)return{ok:false,msg:'Not enough chips to raise.'};p.chips-=add;room.pot+=add;room.betCurrent=target;room.betRaised=true;room.betContrib[p.id]=target;room.betTurn=nextBetIndex(room,room.betTurn)}
...
function botCandidates(hand,current,firstPlay){const candidates=[];const sizes=current?[current.cards.length]:[1,2,3,5];for(const n of sizes){if(n>hand.length)continue;for(const cards of combinations(hand,n)){const v=validPlay(cards,current,firstPlay);if(v.ok)candidates.push({cards,combo:v.combo})}}return candidates}
function chooseBotPlay(room,bot){const hand=sortHand(room.hands[bot.id]||[]),current=room.current,candidates=botCandidates(hand,current,room.firstPlay);if(!candidates.length)return null;candidates.sort((a,b)=>(a.combo.strength??0)-(b.combo.strength??0)||a.cards.length-b.cards.length);return candidates[0]}
...
function botBetTurn(roomId){const room=rooms.get(roomId);if(!room||!room.betting)return;const p=room.players[room.betTurn];if(!p?.isBot)return;let action='call';if((room.betContrib[p.id]||0)>=room.betCurrent&&!room.betRaised)action='raise';else if(Math.random()<0.08)action='fold';const r=handleBet(room,p,action);if(r.ok){broadcastState(room);scheduleBot(room)}}
...
io.on('connection',socket=>{
  ...
  socket.on('room:addBot',()=>{const room=rooms.get(socket.data.room);if(!room)return;if(room.players[0]?.id!==socket.id)return socket.emit('error:msg','Only the host can add a bot.');if(room.started)return socket.emit('error:msg','Game already started.');if(room.players.length>=4)return socket.emit('error:msg','Room is full.');const id=`bot-${room.id}-${Date.now()}-${Math.random().toString(36).slice(2,6)}`;room.players.push({id,name:`Bot ${nextBotNumber(room)}`,ready:true,isBot:true,eliminated:false,folded:false,chips:STARTING_CHIPS});room.hands[id]=[];broadcastState(room)});
