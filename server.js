...
function finishBetting(room){
  ...
  // Pusoy Dos opening turn is ALWAYS the player holding the 3♣.
  // Betting must never overwrite that selector.
...
function handleBet(room,p,action){
  ...
  if(action==='fold'){
    const hasThreeClub=(room.hands[p.id]||[]).some(c=>c.r==='3'&&c.s==='♣');
    if(hasThreeClub)return{ok:false,msg:'The player holding the 3♣ cannot fold before the opening play.'};
...
function chooseBotPlay(room,bot){
  ...
  function playScore(c){
    ...
    const pairBonus=type==='pair'?(isLead?34:18):0;
    const tripleBonus=type==='triple'?(isLead?42:22):0;
