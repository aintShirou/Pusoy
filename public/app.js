const socket=io();let state=null,selected=new Set(),advisorIds=new Set();
...
$('advisorBtn').onclick=()=>{if(!state?.bestPlay?.length)return;advisorIds=new Set(state.bestPlay);renderHand(state.hand);$('advisorBtn').textContent='Highlighted';setTimeout(()=>{$('advisorBtn').textContent='Highlight'},900)};
...
function renderAdvisor(){const r=state?.room,best=state?.bestPlay||[];const show=!!(r?.started&&!r?.betting&&!r?.gameOver&&r.turn===state?.me&&best.length);$('advisor').classList.toggle('hidden',!show);if(!show)return;const cards=state.hand.filter(c=>best.includes(c.r+c.s));$('advisorTitle').textContent=cards.length===1?'Best next card':'Best next play';$('advisorText').textContent=cards.length===1?`Play ${cards[0].r}${cards[0].s}`:`Play ${cards.map(c=>c.r+c.s).join(' · ')}`;advisorIds.clear()}
...
function renderHand(hand){const validIds=new Set(hand.map(c=>c.r+c.s));selected.forEach(id=>{if(!validIds.has(id))selected.delete(id)});advisorIds.forEach(id=>{if(!validIds.has(id))advisorIds.delete(id)});$('handCount').textContent=`${hand.length} card${hand.length!==1?'s':''}`;$('hand').innerHTML=hand.map(c=>cardHTML(c,selected.has(c.r+c.s),advisorIds.has(c.r+c.s))).join('');document.querySelectorAll('#hand .card').forEach(el=>el.onclick=()=>{const id=el.dataset.id;if(selected.has(id))selected.delete(id);else selected.add(id);renderHand(hand)})}
function cardHTML(c,sel=false,best=false){const red=['♥','♦'].includes(c.s);return `<div class="card ${red?'red':''} ${sel?'selected':''} ${best?'bestHint':''}" data-id="${esc(c.r+c.s)}">${best?'<span class="bestBadge">BEST</span>':''}<div class="rank">${esc(c.r)}</div><div class="corner">${esc(c.r)}</div><div class="suit">${esc(c.s)}</div></div>`}
