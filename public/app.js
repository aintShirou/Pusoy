...
function syncRaiseUI(){const r=state?.room;if(!r)return;const min=Math.min(r.maxBet,Math.max(r.minBet,(r.betCurrent||0)+1));const max=r.maxBet;const slider=$('raiseAmount');slider.min=String(min);slider.max=String(max);if(Number(slider.value)<min||Number(slider.value)>max)slider.value=String(min);const v=Number(slider.value);$('raiseAmountValue').value=v;$('raiseButtonAmount').textContent=v;$('raiseMinLabel').textContent=`Min ${min}`;$('raiseMaxLabel').textContent=`Max ${max}`;$('callAmount').textContent=Math.max(0,(r.betCurrent||0)-(r.betContrib?.[state?.me]||0))}
$('raiseAmount').oninput=syncRaiseUI;
$('betRaise').onclick=()=>{const raiseTo=Number($('raiseAmount').value);socket.emit('game:bet',{action:'raise',raiseTo})};
...
function renderRoom(r){
 ...
 $('start').classList.toggle('hidden',!(isHost&&!r.started&&!r.gameOver));$('addBot').classList.toggle('hidden',!(isHost&&!r.started&&r.players.length<4&&!r.gameOver));$('playAgain').classList.toggle('hidden',!(isHost&&r.gameOver));$('playAgain').disabled=!(isHost&&r.gameOver);
 ...
 $('betCall').disabled=!isMyBetTurn||!!r.gameOver;$('betRaise').disabled=!isMyBetTurn||!!r.gameOver||r.betCurrent>=r.maxBet;$('betFold').disabled=!isMyBetTurn||!!r.gameOver;$('betPanel').classList.toggle('hidden',!r.betting);if(r.betting)syncRaiseUI();
