...
$('players').innerHTML=r.players.map(p=>`<div class="player ${p.id===r.turn?'active':''}">${p.isBot?'🤖':'👤'} ${esc(p.name)} · ${p.cardCount} 🂠</div>`).join('');$('turn').textContent=r.winner?'ROUND OVER':(r.started?(r.turn===state?.me?'YOUR TURN':'Waiting for opponent'):'');
...
$('start').classList.toggle('hidden',!(r.players[0]?.id===state?.me&&!r.started));$('addBot').classList.toggle('hidden',!(r.players[0]?.id===state?.me&&!r.started&&r.players.length<4));$('pass').disabled=!r.started||!r.current||r.turn!==state?.me||!!r.winner;$('play').disabled=!r.started||r.turn!==state?.me||!!r.winner;renderPlayed(r.current?.cards||[])}
