...
$('addBot').onclick=()=>socket.emit('room:addBot');
...
const lastFinished=r.winner?r.players.find(p=>p.id===r.winner):null;
$('status').textContent=r.gameOver?'GAME OVER':(r.winner&&r.started?`${lastFinished?.name||'Player'} finished!`:(r.started?'Game in progress':'Waiting for players'));
$('players').innerHTML=r.players.map(p=>`<div class="player ${p.id===r.turn&&!p.eliminated?'active':''} ${p.eliminated?'eliminated':''}">${p.isBot?'🤖':'👤'} ${esc(p.name)} ${p.eliminated?'✓':''} · ${p.cardCount} 🂠</div>`).join('');
$('turn').textContent=r.gameOver?'GAME OVER':(r.started?(r.turn===state?.me?'YOUR TURN':'Waiting for opponent'):'');
const finishNames=(r.finished||[]).map(id=>r.players.find(p=>p.id===id)?.name).filter(Boolean);
$('hint').textContent=r.gameOver?`Game over. Finish order: ${finishNames.join(' → ')}`:(r.winner&&r.started?`${lastFinished?.name||'Player'} emptied their hand and is out! Continue until one player remains.`:(r.started?(r.current?`Beat ${r.current.combo.type} · ${r.current.cards.length} card${r.current.cards.length>1?'s':''}`:'Lead any valid combination'):'Host can start once at least 2 players join.')); 
$('start').classList.toggle('hidden',!(r.players[0]?.id===state?.me&&!r.started&&!r.gameOver));$('addBot').classList.toggle('hidden',!(r.players[0]?.id===state?.me&&!r.started&&r.players.length<4&&!r.gameOver));$('playAgain').classList.toggle('hidden',!(r.players[0]?.id===state?.me&&r.gameOver));$('pass').disabled=!r.started||!r.current||r.turn!==state?.me||!!r.gameOver;$('play').disabled=!r.started||r.turn!==state?.me||!!r.gameOver;renderPlayed(r.current?.cards||[])}
